...
var sanitizeNames = _.compose(map(_.compose(_underscore, toLowerCase)), map(_.prop('name')));
sanitizeNames = map(_.compose(_underscore, toLowerCase, _.prop('name')));
...
var formatDollars = _.compose(accounting.formatMoney, _.prop('dollar_value'));
availablePrices  = _.compose(join(', '), map(formatDollars), _.filter(_.prop('in_stock')));
...
var fastestCar2 = _.compose(_. last, _.sortBy(_.prop('horsepower')));
var extension = ' is the fastest';
var flippedConcat = _.flip(_.concat);
fastestCar = _.compose(flippedConcat(extension), _.prop('name'), fastestCar2);
